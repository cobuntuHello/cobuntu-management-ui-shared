...
import type { DonationDraft, SymbolFor } from "./types";
import { DonationsSection } from "./DonationsSection";
import { Switch } from "./primitives";

/**
 * The responsive modal shell to open the settings in.
 *
 * Injected rather than taken from this package, and that is load-bearing. Each
 * management package has its OWN ModalShell: a centred dialog on desktop and a
 * bottom sheet with drag-to-dismiss on mobile. This package's own ModalShell is
 * a different component with a different API (title/subtitle/footer slots).
 * Using it here would have quietly replaced the mobile drawer on both apps,
 * which is not a change this consolidation is entitled to make.
 *
 * Both packages' shells already match this signature exactly.
 */
export type DonationsModalShell = React.ComponentType<{
  onClose: () => void;
  width?: string;
...
export interface DonationsFieldProps {
  ...
  /** Currency code -> display symbol. See SymbolFor in ./types. */
  symbolFor: SymbolFor;
  /** "variant" for products, "ticket tier" for events. See DonationsSection. */
  tierNoun: string;
  /** The calling package's own responsive shell. */
  modalShell: DonationsModalShell;
}
 ...
 * the tiers card and why one prompt covers every tier. The settings
...
export function DonationsField({
  donation, onUpdate, defaultCurrency, symbolFor, tierNoun, modalShell: ModalShell,
}: DonationsFieldProps) {
  const [open, setOpen] = useState(false);
  const sym = symbolFor(donation.currency || defaultCurrency);
        ...
        <ModalShell onClose={() => setOpen(false)} width="w-[520px]">
          ...
          <div className="px-1">
            ...
            <p className="text-[12px] text-zinc-500 mt-0.5 leading-snug">
              An optional contribution at checkout, on top of any price. The same prompt shows for every {tierNoun}.
            </p>
          </div>
