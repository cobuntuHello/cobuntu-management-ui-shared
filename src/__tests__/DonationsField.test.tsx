/**
 * Donations on the create-wizard pricing step — now owned by THIS package.
 *
...
import { DonationsField } from "../donations";
...
/** Stand-in for a package's responsive ModalShell: renders inline, no portal. */
...
const symbolFor = (code: string) => SYMBOLS[code] ?? code;

function setup(over: Partial<DonationDraft> = {}) {
  const onUpdate = vi.fn();
  render(
    <DonationsField
      donation={draft(over)} onUpdate={onUpdate} defaultCurrency="EUR"
      symbolFor={symbolFor} tierNoun="variant" modalShell={TestShell}
    />,
  );
...
describe("DonationsField — the modal, once there is something in it", () => {
  ...
  it("turning donations OFF while the editor is open closes it", async () => {
    ...
    const { rerender } = render(
      <DonationsField
        donation={draft({ enabled: true, amounts: ["5"] })} onUpdate={onUpdate} defaultCurrency="EUR"
        symbolFor={symbolFor} tierNoun="variant" modalShell={TestShell}
      />,
    );
    ...
    rerender(
      <DonationsField
        donation={draft({ enabled: false })} onUpdate={onUpdate} defaultCurrency="EUR"
        symbolFor={symbolFor} tierNoun="variant" modalShell={TestShell}
      />,
    );
    expect(screen.queryByText(/how buyers give/i)).not.toBeInTheDocument();
